#include <stdio.h>
#include "dbg.h"
#include <stdarg.h>
#include <stdlib.h>

#define MAX_DATA 100

int string_read( char **out_string, int  max_buffer)
{
	*out_string = calloc(1, max_buffer+1);
	char *ptr = *out_string;
	check_mem(*out_string);
	int i = 0;
	char result;

	//char *result = fgets(*out_string, max_buffer, stdin);	
	//check(result != NULL, "Failed input");

	for(i=0;i<max_buffer+1;i++){
		result = fgetc(stdin);
			if(result == '\n') break;
			*ptr++ = result;

		}
			*ptr = '\0';
		 
	
	return 0;

error:
	if(out_string){
		free(out_string);}
	*out_string = NULL;
	return -1;

}

int read_int(int *out_int)
{
	char *input = NULL;
	int rc = string_read(&input, MAX_DATA);
	check(rc == 0, "Failed to read number");

	*out_int = atoi(input);
	free(input);
	return 0;
error:
	if(input) free(input);
	return -1;
}


int read_scan(const char *fmt, ...)
{
	int i = 0;
	int rc = 0;
	char **out_string = NULL;
	char *out_char = NULL;
	int *out_int = NULL;
	int max_buffer = 0;

	va_list argp;
	va_start(argp, fmt);

	for(i = 0; fmt[i] != '\0'; i++){
		if(fmt[i] == '%'){
			i++;
		switch (fmt[i]){
		case '\0':
			sentinel("You ended with %%");
			break;
		case 'd':
			out_int = va_arg(argp, int *);
			rc = read_int(out_int);
			check(rc == 0, "Failed to read int");			
			break;
		case 'c':
			out_char = va_arg(argp, char *);
			*out_char = fgetc(stdin);
			break;
		case 's':
			max_buffer = va_arg(argp, int);
			out_string = va_arg(argp, char**);
			rc = string_read(out_string, max_buffer);
			check(rc == 0, "Failed to read string");
			break;
		default:
			sentinel("Invalid format");
		}
		}else{
			fgetc(stdin);
		}
	
	
	check( !feof(stdin) && !ferror(stdin), "Input error");
	}
	va_end(argp);

	return 0;
error:
	va_end(argp);
	return -1;
}

void prt_scr(const char *string, ...)
{

	char *out_string = NULL;

	va_list(argp);
	va_start(argp, string);
	printf("%s ", string);

	while(1){
		out_string = va_arg(argp, char*);
		if(out_string == NULL) break;
		printf("%s\n", out_string);
	}

	va_end(argp);
		
}
					
		


int main( int argc, char *argv[])
{
	char *last_name = NULL;
	char initial = ' ';
	char *first_name = NULL;
	int age = 0;


	printf("What is your first name? ");
	int rc = read_scan("%s", MAX_DATA, &first_name);
	check(rc == 0, "Failed to read last name");

	printf("What is your initial? ");
	rc = read_scan("%c\n", &initial);
	check(rc == 0, "Failed to read initial");

	printf("What is your last name?");
	rc = read_scan("%s", MAX_DATA, &last_name);
	check(rc == 0, "Failed to read last name");

	printf("What is your age?");
	rc = read_scan("%d", &age);
	check(rc == 0, "Failed to read age");

	printf("-----------RESULTS------------\n");
	printf("First name: %s\n", first_name);
	printf("Initials: %c\n", initial);
	printf("Last name: %s\n", last_name);
	printf("Age: %d\n", age);

	prt_scr("The list: ", last_name, first_name, first_name);
	
	free(first_name);
	free(last_name);

	return 0;
error: 
	return -1;
}
